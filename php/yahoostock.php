...
foreach( $objYahooStock->getQuotes() as $code => $stock)
{
  /*
   ...
   //$sql = "insert into CurrentStockPricing (CompanyName, Ticker, Price, Date, Time, ChangePercent, Volume) values ($stock[1], $stock[0], $stock[2], $stock[3], $stock[4], $stock[5], $stock[6]);";
    //$sql = "insert into StockInfo (CompanyName, Ticker) values ($stock[1], $stock[0]);";
   $sql = "UPDATE CurrentStockPricing SET Price=$stock[2],Date=$stock[3],Time=$stock[4],ChangePercent=$stock[5],Volume=$stock[6] where Ticker like $stock[0];";
   echo "<p>" . $sql . "</p>";
